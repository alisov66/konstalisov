...
function SkillTags({ tags }: { tags: string[] }) {
  return (
    <div className="flex w-full max-w-[820px] flex-wrap gap-[var(--base-2)]">
      {tags.map((tag) => (
        <span
          className="inline-flex shrink-0 items-center justify-center whitespace-nowrap rounded-[var(--pill)] border border-[var(--border-secondary)] px-[var(--base-3)] py-[var(--base-1)] text-[var(--text-primary)] [word-break:break-word]"
          key={tag}
...
export default function CvPage() {
  return (
    <main className="min-h-screen bg-[var(--bg-beige)] text-[var(--text-primary)]">
      ...
      <section className="flex w-full flex-col gap-[var(--base-20)] px-[var(--padding-side)] pb-[var(--base-6)] pt-[var(--base-30)]">
